public class SegmentTree {
    ...
    /**
     * Currently implemented for SUMS
     * 
    ...
    void set(int idx, int val, int x, int l_idx, int r_idx) {
        ...
        vals[x] = vals[2 * x + 1] + vals[2 * x + 2]; // TODO: Change calculation here
    }
    ...
    long calc(int left, int right, int x, int l_idx, int r_idx) {
        if (l_idx >= right || left >= r_idx)
            return Integer.MIN_VALUE; // TODO: Change calculation here
        if (l_idx >= left && r_idx <= right)
            return vals[x];

        int m_idx = (l_idx + r_idx) / 2;
        long left_sum = calc(left, right, 2 * x + 1, l_idx, m_idx);
        long right_sum = calc(left, right, 2 * x + 2, m_idx, r_idx);
        return Math.min(left_sum, right_sum); // TODO: Change calculation here
    }
    ...
    void build(int[] a, int x, int l_idx, int r_idx) {
        ...
        vals[x] = Math.max(vals[2 * x + 1], vals[2 * x + 2]); // TODO: Change calculation here
    }
